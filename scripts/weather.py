...
def current_weather():
    ...
    print_entry('rain',     f'\x1b[34m{rainfall} in\x1b[0m')
...
def daily_forecast():
    ...
    for day in sorted(daily_data):
        ...
        print_entry('rain',     f'\x1b[34m{data["rainfall_total_estimated"]} in\x1b[0m' +
...
def detailed_forecast():
    ...
    for day in sorted(res):
        ...
        for entry in res[day]:
            ...
            if rainfall > 0:
                output += f'\x1b[90m: \x1b[34m{rainfall} in \x1b[90m({precipitation_prob}%)\x1b[0m'
...
def waybar_current(data: dict) -> str:
    ...
    output += waybar_entry('rain',     colorize(f"{rainfall} in", BLUE))
    return output
    ...
    for day in sorted(daily_data):
        ...
        if data['rainfall_total_estimated'] > 0:
            line_content += colorize(': ', GRAY) + \
                            colorize(f'{data["rainfall_total_estimated"]} in ', BLUE) + \
